...
TEST(CoreTest, UTF8Validator)
{
	...
	ASSERT_TRUE(os::ValidateUTF8(os::ToUTF8(os::unicode_string(L"觇痂腚桷?"))));
	ASSERT_FALSE(os::ValidateUTF8(std::string("觇痂腚桷?1251")));
}
TEST(CoreTest, FromACP)
{
	ASSERT_TRUE(os::FromACP(std::string("觇痂腚桷?1251")) == os::unicode_string(L"觇痂腚桷?1251"));
}
#endif